...
def _db(path:Path):
    ...
    create table if not exists symbols(path text,qualname text,name text,kind text,signature text,start_line int,end_line int,decorators text,bases text,docstring text, primary key(path, qualname));
...
def build_index(repo:Path,out:Path):
    ...
    for p in py:
        ...
        for s in syms:
            cur.execute('insert or replace into symbols values (?,?,?,?,?,?,?,?,?,?)',(rel,s.qualname,s.name,s.kind,s.signature,s.start_line,s.end_line,json.dumps(s.decorators),json.dumps(s.bases),s.docstring))
...
def _ensure_git_ref(repo: Path, ref: str) -> None:
    """Ensure a git ref exists locally; fetch PR refs when possible."""
    chk = subprocess.run(["git", "-C", str(repo), "rev-parse", "--verify", ref], capture_output=True, text=True, check=False)
    if chk.returncode == 0:
        return
    m = re.fullmatch(r"pr-(\d+)", ref)
    if m:
        pr = m.group(1)
        subprocess.run(["git", "-C", str(repo), "fetch", "origin", f"pull/{pr}/head:{ref}"], capture_output=True, text=True, check=False)
        chk2 = subprocess.run(["git", "-C", str(repo), "rev-parse", "--verify", ref], capture_output=True, text=True, check=False)
        if chk2.returncode == 0:
            return
    raise RuntimeError(
        f"Git ref '{ref}' not found in {repo}. "
        f"If this is a PR ref, run: git -C {repo} fetch origin pull/<PR_NUMBER>/head:{ref}"
    )


def _diff(repo,base,head):
    _ensure_git_ref(repo, base)
    _ensure_git_ref(repo, head)
    p = subprocess.run(["git","-C",str(repo),"diff","--unified=0",base,head],capture_output=True,text=True,check=False)
    if p.returncode != 0:
        raise RuntimeError(f"git diff failed for {base}..{head}: {p.stderr.strip()}")
    return p.stdout
...
def main():
    ...
    try:
        if a.cmd=='index':
            build_index(repo,out); print(f"Index written to {out/'python_index.json'} and {out/'review_index.db'}")
        else:
            review(repo,out,a.base,a.head); print(f"Review report written to {out/'review_report.json'}")
