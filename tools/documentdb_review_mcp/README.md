...
## Git refs for PR review

If `--head` is a PR-style ref (e.g., `pr-21209`), the tool now attempts:

```bash
git -C <repo> fetch origin pull/21209/head:pr-21209
```

If refs are still missing, it exits with a friendly message describing the exact fetch command.
